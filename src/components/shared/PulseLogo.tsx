...
export function PulseLogo({ size = 'md', showText = true, className }: PulseLogoProps) {
  const sizes = {
  ...
  return (
    <div className={cn('flex items-center gap-3', className)}>
      <div className={cn('relative rounded-2xl primary-gradient shadow-sm flex items-center justify-center', s.container)}>
        {/* Heart with EKG */}
        <svg
          viewBox="0 0 40 40"
          className={s.icon}
        ...
        {/* Pulse glow effect */}
        <div className="absolute inset-0 rounded-2xl primary-gradient opacity-50 animate-pulse-glow" />
      </div>
      
      {showText && (
        <span className={cn('font-bold text-sidebar-foreground', s.text)}>
          PulseCare
        </span>
      )}
    </div>
